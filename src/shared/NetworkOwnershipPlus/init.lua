...
function NetworkOwnershipPlus.CreateEntity(Name: string, Angle: Vector3, Position: Vector3, Owner: Player?, ...): Types.Entity
    ...
    if Owner then
        NetworkOwnershipPlus.SetEntityNetworkOwner(Entity, Owner)
    end

    return Entity
end

function NetworkOwnershipPlus.DestroyEntity(Entity: Types.Entity)
    --> Remove from records
    EntitySlots[Entity.Identifier] = nil

    --> Remove network ownership
    if Entity.Owner then
        NetworkOwnershipPlus.SetEntityNetworkOwner(Entity)
    end
...
function NetworkOwnershipPlus.RegisterEntity(Definition: Types.EntityDefinition)
    EntityDefinitions[Definition.Name] = Definition
end

function NetworkOwnershipPlus.SetEntityNetworkOwner(Entity: Types.Entity, Owner: Player?)
