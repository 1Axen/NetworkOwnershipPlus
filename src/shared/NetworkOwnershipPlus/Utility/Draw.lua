...
function Draw.clear()
	Draw.getDefaultParent():ClearAllChildren()
...
function Draw.capsule(Position: Vector3, Radius: number, Height: number, Color: Color3?, Parent: Instance?): BasePart
	if true then
		return Draw.box(CFrame.new(Position), Vector3.new(2, 5, 2), Color)
	end
    ...
    Wireframe.ZIndex = 1
    Wireframe.Color3 = Color or Draw._defaultColor
    ...
    local Slices = 16
    local SliceSize = math.rad(360 / Slices)
    local Rotations = {
        0,
        math.rad(90),
    }

    local function DrawSphere(Half: number)
		for Rotation = 1, (#Rotations + 1)  do
			local Previous;
			for Slice = 0, Slices do 
                local Point: Vector3;
				local Angle = (SliceSize * Slice)

				if Rotation <= #Rotations then
					if (Half == 1 and Angle < math.pi) 
						or (Half == -1 and Angle > math.pi) then
						continue
					end

					Point = (CFrame.Angles(0, Rotations[Rotation], 0) * CFrame.Angles(Angle, 0, 0) * CFrame.new(0, 0, -Radius)).Position
				else
					Point = (CFrame.Angles(0, Angle, 0) * CFrame.new(0, 0, -Radius)).Position
				end
				
				Point -= Vector3.new(0, Height * Half, 0)
				...
				Wireframe:AddLine(Previous, Point)
				Previous = Point
			end
		end
	end
    ...
    local function DrawCylinder()
		for Index = 0, 3 do
			local Angle = math.rad(90 * Index)
			local Top = Vector3.new(0, Height, 0) + (CFrame.Angles(0, Angle, 0) * CFrame.new(0, 0, -Radius)).Position
			local Bottom = Vector3.new(0, -Height, 0) + (CFrame.Angles(0, Angle, 0) * CFrame.new(0, 0, -Radius)).Position
			Wireframe:AddLine(Top, Bottom)
		end
	end
    ...
    --> Parent to datamodel
    Part.Parent = Parent or Draw.getDefaultParent()
